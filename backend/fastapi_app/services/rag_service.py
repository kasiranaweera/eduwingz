...
class RAGService:
    ...
    async def initialize(self):
        if self._initialized:
            return
        try:
            # Load HuggingFace text-generation pipeline for reasoning models
            ...
            self.llm = pipeline(
                ...
                device=device,
                max_new_tokens=512,  # Increased for better reasoning output
                do_sample=True,
    ...
    async def chat(self, message: str, session_id: str) -> Dict:
        try:
            ...
            def generate_response():
                outputs = self.llm(prompt, max_new_tokens=512, do_sample=True, temperature=0.7, top_p=0.9)
